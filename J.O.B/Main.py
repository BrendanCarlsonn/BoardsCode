...
async def help(ctx):
    ...
    embed.add_field(name='!changeJob', value='Change your job searching roles by using !changeJob @your server name role. EX !changeJob @J.O.B Computer Science', inline=False)
    embed.add_field(name='!displaySearch', value='Have job links delivered in a matter of seconds with !displaySearch. Make sure you have set your role first!', inline=False)
    embed.add_field(name='!sendJob', value='Found a job you think would better suit a friend? !sendJob @username. EX !sendJob @J.O.B', inline=False)
    embed.add_field(name='Misc', value='!info (displays user info), !giverole (manually gives a role), !list(arguments are listed in a discord message)', inline=False)
...
bot.run('OTU1MTU3NzgzOTk5ODM2MTYw.GM07K9.66_imaLTHEUpjz2SBrXu1PtLvYg1fUAtpGtszI')